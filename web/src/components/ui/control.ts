...
/**
 * What a control looks like in this program.
 *
 * Here rather than beside the Button component because one thing in the program is a
 * link and looks like a control — the coffee, which goes somewhere rather than doing
 * something — and a file that exports a component may export nothing else.
 *
 * The components under ui/ are copied into this repository rather than imported from
 * a package. That is deliberate: nothing is fetched at build time or at run time that
 * is not in the repository, and a component can be adapted to this archive's
 * vocabulary instead of the archive being bent to a library's.
 */
